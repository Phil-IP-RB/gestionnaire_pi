...
class GestionnairePiDockWidget(QtWidgets.QDockWidget, FORM_CLASS):
    ...
    def __init__(self, plugin):
        ...
        QTimer.singleShot(0, self._shrink_combos)
    ...
    # ─── Compactage des QComboBox ────────────────────────────────────
    def _shrink_combos(self):
        """Réduit la largeur mini imposée par les QComboBox + fixe
        une largeur minimale confortable pour tout le dock."""
        from qgis.PyQt.QtCore import Qt

        for c in self.findChildren(QtWidgets.QComboBox):
            c.setSizeAdjustPolicy(c.AdjustToMinimumContentsLengthWithIcon)
            c.setMinimumContentsLength(1)               # 1 caractère suffit
            c.view().setTextElideMode(Qt.ElideMiddle)   # coupe au milieu

        self.setMinimumWidth(160)   # ← ajuste la valeur à ton goût
        self.adjustSize()           # force Qt à recalculer les hints
