...
def georef_work_code(value) -> str:
    """Renvoie la nature des travaux à partir du code qualité."""
    try:
        letter = str(value).strip().upper()[0]
    except Exception:
        return '-'
    
    if letter in ('0', 'F', 'G', 'H', 'I', 'J', 'P', 'X'):
        return 'Transformation Helmert'
    elif letter in ('C', 'L'):
...
def process_data(
    ...
    vrais, raccords, corrections, georef = [], [], [], []
    ...
    for f in folio_layer.getFeatures():
        ...
        # Ajout dans georef si qualite_no commence par autre chose que D ou E
        qual = str(f['qualite_no']).strip().upper()
        if qual and qual[0] not in ('D', 'E'):
            georef.append(f)
...
def generate_csv_files(corrections: List[QgsFeature],
    ...
    try:
        ...
        paths = {
            ...
            'georef':     os.path.join(output_folder, 'georef.csv')
        }
        for key in ('folios', 'atlas'):
            if os.path.exists(paths[key]):
                os.remove(paths[key])
        if corrections and os.path.exists(paths['correction']):
            os.remove(paths['correction'])
        ...
        folio_csv_fields = {
            ...
            'Plan': 'plan_nom',
            'Code qualité du plan': 'qualite_no',
                    ...
                    if field in ('lg_res_clc', 'lg_res_clb'):
                        if feat['type'] == 'vrai' and raw not in ('', None):
                            try:
                                ...
                                val = '' if raw_float == 0 else f"{raw_float:.1f}"
        ...
        # ---------- corrections.csv (si corrections non vides) -------------
        if corrections:
            with open(paths['correction'], 'w', newline='', encoding='utf-8') as f:
                ...
                for feat in corrections:
                    ...
                    for fld in fields:
                        raw = feat[fld]
                        if fld in ('lg_res_clc', 'lg_res_clb'):
                            try:
                                raw_float = float(str(raw).replace(',', '.'))
                                val = '' if raw_float == 0 else f"{raw_float:.1f}"
                            except (ValueError, TypeError):
        ...
        # ---------- georef.csv (si georef non vide) -------------------------
        if georef:
            with open(paths['georef'], 'w', newline='', encoding='utf-8') as f:
                f.write('\ufeff')
                w = csv.writer(f, delimiter=';')
                w.writerow([
                    'Plan',
                    'Code qualité de géoréférencement du plan initial',
                    'Commune',
                    'Code INSEE',
                    'Rue concernée',
                    'Corine Land Cover',
                ...
                for feat in georef:
                    if clean_value(feat['type']).lower() != 'vrai':
                        continue
                    w.writerow([
                        f"'{clean_value(feat['plan_nom'])}",
                        clean_value(feat['qualite_no']),
                        clean_value(feat['commune_no']),
                        f"'{clean_value(feat['commune_in'])}",
                        clean_value(feat['voie_princ']),
                        clean_value(feat['corine']) if 'corine' in feat.fields().names() else '',
                        georef_work_code(feat['qualite_no']) if 'qualite_no' in feat.fields().names() else '*'
                    ])
