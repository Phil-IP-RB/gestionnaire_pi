...
class Annexe6Processor:
    ...
    def run_custom(
        ...
        # Enregistrer automatiquement les modifications en cours
        def enregistrer_modifications(layer):
            if layer and layer.isEditable():
                success = layer.commitChanges()
                if not success:
                    QMessageBox.warning(None, "Erreur", f"Échec de l'enregistrement pour : {layer.name()}")

        # Appliquer sur les 3 couches concernées
        enregistrer_modifications(line_layer)
        enregistrer_modifications(detection_zone_layer)
        enregistrer_modifications(folio_layer)
        ...
        # ------------------------------------------------------------------
        while True:
            total_zones, length_c, length_b, length_w, corrections, folios, raccords, georef = process_data(
                line_layer,
            ...
            dlg = ValidationDialog(
                total_zones, round(length_c, 1), round(length_b, 1), round(length_w, 1)
            )
            ...
            if generate_csv_files(
                corrections, folios, raccords, folio_layer, output_folder, georef
            ):
                QMessageBox.information(
